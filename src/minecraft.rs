...
//pub async fn get_updates_old(data: &ShardData) -> Result<Vec<MemberUpdate>, UpdateRolesError> {
//    let first = data.hypixel.get_guild(MAIN_GUILD_NAME).await?.members;
//    let second = data.hypixel.get_guild(SECOND_GUILD_NAME).await?.members;
//
//    let list_of_uuids: Vec<&String> = first.iter().chain(second.iter()).map(|x| &x.uuid).collect();
//    let linked_users = get_linked_users_by_uuids(&data.pool, list_of_uuids).await?;
//
//    let guild_members: Vec<Member> = get_guild_members(data).await?;
//
//    let first_role = get_guild_role(MAIN_GUILD_NAME);
//    let second_role = get_guild_role(SECOND_GUILD_NAME);
//
//    let mut updates = Vec::new();
//
//    for member in guild_members {
...
//                        .map(|x| (x, second_role))
//                });
//            if let Some((guild_user, guild_role)) = data {
//                to_add.push(guild_role);
//                let role = get_guild_rank_role(MAIN_GUILD_NAME, &guild_user.rank);
//                let all_roles = get_guild_all_rank_roles(MAIN_GUILD_NAME);
//                if let Some(role) = role {
//                    let all_roles = all_roles.unwrap();
//                    to_add.push(role);
//                    to_remove.extend(all_roles.iter().copied().filter(|x| *x != role))
//                } else if let Some(all_roles) = all_roles {
//                    to_remove.extend(all_roles);
//                }
//            }
//        } else {
//            to_remove.reserve(3);
//            to_remove.push(first_role);
//            to_remove.push(second_role);
//            to_remove.push(data.config.verified_role_id);
//        }
//
//        let added: Vec<Id<RoleMarker>> = to_add
